...
test('identity comparison is a signal, not verification',()=>{assert.deepEqual(compareIdentities('Jane.Doe','janedoe').signals,['NORMALIZED_MATCH']);assert.equal(compareIdentities('Jane.Doe','janedoe').score,55);});
test('geo conversion and distance are deterministic',()=>{assert.equal(dmsToDecimal(45,0,0,'N'),45);assert.deepEqual(decimalToDms(-45.5,'lat'),{degrees:45,minutes:30,seconds:0,hemisphere:'S'});const r=geoDistanceBearing(45,0,45,1);assert.ok(r.distanceKm>70&&r.distanceKm<80);assert.ok(r.bearing>80&&r.bearing<100);assert.throws(()=>geoDistanceBearing(91,0,45,1),/out of range/);});
